...
ENV HF_HOME=/scratch/sh57680/hf_cache
ENV HF_HUB_CACHE=/scratch/sh57680/hf_cache/hub
ENV HF_XET_CACHE=/scratch/sh57680/hf_cache/xet
...
WORKDIR /workspace/Med3DVLM
ENV PYTHONPATH=/workspace/Med3DVLM:$PYTHONPATH

# ------------------------------------------------------------
# Install PyTorch first
# This is important because causal-conv1d setup imports torch.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Install causal-conv1d after torch exists
# causal-conv1d is a CUDA/PyTorch extension.
# ------------------------------------------------------------
RUN python -m pip install --no-cache-dir --no-build-isolation \
    causal-conv1d==1.5.0.post8

# ------------------------------------------------------------
# Install remaining Med3DVLM dependencies
# ------------------------------------------------------------
RUN python -m pip install --no-cache-dir \
    SimpleITK==2.4.1 \
    Unidecode==1.3.8 \
